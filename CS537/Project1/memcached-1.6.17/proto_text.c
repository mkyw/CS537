...
static void process_arithmeticx_command(conn *c, token_t *tokens, const size_t ntokens, const bool incr) {
    char temp[INCR_MAX_STORAGE_LEN];
    uint64_t delta;
    char *key;
    size_t nkey;
...
void process_command_ascii(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        if (strcmp(tokens[COMMAND_TOKEN].value, "mult") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_arithmeticx_command(c, tokens, ntokens, 1);
        } else {
            out_string(c, "ERROR");
        }
    } else if (first == 'd') {
        if (strcmp(tokens[COMMAND_TOKEN].value, "delete") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_arithmeticx_command(c, tokens, ntokens, 0);
